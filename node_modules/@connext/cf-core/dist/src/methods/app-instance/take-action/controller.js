    ...
    async beforeExecution(requestHandler, params) {
        ...
        try {
            ...
            if (e.code === errors_1.INVALID_ARGUMENT) {
                throw Error(`${errors_2.IMPROPERLY_FORMATTED_STRUCT}: ${utils_1.prettyPrintObject(e)}`);
            }
...
__decorate([
    rpc_server_1.jsonRpcMethod(types_1.Node.RpcMethodName.TAKE_ACTION),
...
async function runTakeActionProtocol(appIdentityHash, store, protocolRunner, initiatorXpub, responderXpub, action) {
    ...
    try {
        ...
        if (e.toString().indexOf("VM Exception") !== -1) {
            throw Error(`${errors_2.INVALID_ACTION}: ${utils_1.prettyPrintObject(e)}`);
        }
        throw Error(`Couldn't run TakeAction protocol: ${utils_1.prettyPrintObject(e)}`);
    }
    return {};
}
