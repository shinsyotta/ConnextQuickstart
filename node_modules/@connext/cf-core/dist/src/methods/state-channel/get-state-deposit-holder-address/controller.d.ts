import { RequestHandler } from "../../../request-handler";
import { Node } from "../../../types";
import { NodeController } from "../../controller";
export default class GetStateDepositHolderAddressController extends NodeController {
    executeMethod: (requestHandler: RequestHandler, params: Node.MethodParams) => Promise<Node.MethodResult>;
    protected executeMethodImplementation(requestHandler: RequestHandler, params: Node.GetStateDepositHolderAddressParams): Promise<Node.GetStateDepositHolderAddressResult>;
}
//# sourceMappingURL=controller.d.ts.map