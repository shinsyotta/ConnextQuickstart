...
async function makeDeposit(requestHandler, params) {
    ...
    while (retryCount > 0) {
        try {
            ...
            if (e.toString().includes("reject") || e.toString().includes("denied")) {
                outgoing.emit(types_1.NODE_EVENTS.DEPOSIT_FAILED, failMsg);
                throw Error(`${errors_1.DEPOSIT_FAILED}: ${utils_2.prettyPrintObject(e)}`);
            }
            retryCount -= 1;
            if (retryCount === 0) {
                outgoing.emit(types_1.NODE_EVENTS.DEPOSIT_FAILED, failMsg);
    ...
    outgoing.emit(types_1.NODE_EVENTS.DEPOSIT_STARTED, {
        from: publicIdentifier,
        type: types_1.NODE_EVENTS.DEPOSIT_STARTED,
        data: {
            value: amount,
