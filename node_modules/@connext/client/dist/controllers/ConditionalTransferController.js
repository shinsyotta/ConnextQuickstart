...
class ConditionalTransferController extends AbstractController_1.AbstractController {
    constructor() {
        ...
        this.resolveInstallTransfer = (res, appId, message) => {
            const appInstance = message.data.data ? message.data.data : message.data;
            if (appInstance.identityHash !== appId) {
                this.log.warn(`Caught INSTALL event for different app ${lib_1.stringify(message)}, expected ${appId}. This should not happen.`);
                return;
            }
            res(message);
            return message;
        ...
        this.rejectInstallTransfer = (rej, appId, msg) => {
            if (appId !== msg.data.appInstanceId) {
                this.log.warn(`Caught INSTALL event for different app ${lib_1.stringify(msg)}, expected ${appId}. This should not happen.`);
                return;
            }
