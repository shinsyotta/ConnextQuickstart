...
describe("Scenario: install AppInstance, set state, put on-chain", () => {
    it("returns the funds the app had locked up for both ETH and ERC20 in app and free balance", async (done) => {
        ...
        proxyFactory.once("ProxyCreation", async (proxyAddress) => {
            let stateChannel = models_1.StateChannel.setupChannel(network.IdentityApp, proxyAddress, xpubs, 1).setFreeBalance(free_balance_1.FreeBalanceClass.createWithFundedTokenAmounts(multisigOwnerKeys.map(key => key.address), constants_1.WeiPerEther, [constants_2.CONVENTION_FOR_ETH_TOKEN_ADDRESS, erc20TokenAddress]));
